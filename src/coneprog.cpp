 ...
 */
void coneqp(Matrix &P, Eigen::VectorXd &q, Matrix *G = NULL, Vector* h = NULL, Vector* dims = NULL, Vector*A = NULL, Vector*b = NULL,
    Vector* initvals = NULL, Vector* kktsolver = NULL, Vector* xnewcopy = NULL, Vector* xdot = NULL,
    Vector* xaxpy = NULL, Vector* xscal = NULL, Vector* ynewcopy = NULL, Vector* ydot = NULL, Vector* yaxpy = NULL,
            Vector* yscal = NULL, const ConeqpOptions &options = ConeqpOptions()){
    ...
    if (not matrixP or (not matrixG and G is not NULL) or 
        (not matrixA and A is not NULL)) and not customkkt:
        raise ValueError("use of function valued P, G, A requires a "\
            "user-provided kktsolver")
    customx = (xnewcopy != NULL or xdot != NULL or xaxpy != NULL or 
        xscal != NULL) 
    ...
    customy = (ynewcopy != NULL or ydot != NULL or yaxpy != NULL or 
        yscal != NULL) 
    ...
    if xnewcopy is NULL: xnewcopy = matrix 
    if xdot is NULL: xdot = blas.dot
    if xaxpy is NULL: xaxpy = blas.axpy 
    if xscal is NULL: xscal = blas.scal 
    def xcopy(x, y): 
        xscal(0.0, y) 
        xaxpy(x, y)
    if ynewcopy is NULL: ynewcopy = matrix 
    if ydot is NULL: ydot = blas.dot 
    if yaxpy is NULL: yaxpy = blas.axpy 
    if yscal is NULL: yscal = blas.scal
    def ycopy(x, y): 
        yscal(0.0, y) 
        yaxpy(x, y)
    ...
    for iters in xrange(options.maxiters + 1):
        ...
        if ( pres <= options.feastol and dres <= options.feastol and ( gap <= options.abstol or 
            (relgap is not NULL and relgap <= options.reltol) )) or \
            iters == options.maxiters:
            ind = dims['l'] + sum(dims['q'])
            for m in dims['s']:
                misc.symm(s, m, ind)
                misc.symm(z, m, ind)
